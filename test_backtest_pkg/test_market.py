...
class TestMarketSingleAsset(unittest.TestCase):
    ...
    def test_market_RSI_down(self):
        expect = pd.DataFrame(0., index=[self.index[-1]], columns=self.ticker)
    ...
    def test_market_oscillator(self):
        expect = pd.DataFrame((3-0.9)/(5.5-0.9), index=[self.index[-1]], columns=self.ticker)
        assert_frame_equal(self.market_sin.oscillator(), expect)
    def test_market_oscillator_given_date(self):
        date_str = '2020-01-03'
        date = pd.to_datetime(date_str)
        expect = pd.DataFrame((3-2.7)/(5.5-2.7), index=[date], columns=self.ticker)
    ...
    def test_market_oscillator_concave(self):
        expect = pd.DataFrame((1-0.9)/(3.3-0.9), index=[self.index[-1]], columns=self.ticker)
...
class TestMarketMultiAsset(unittest.TestCase):
    ...
    def test_market_daily_ret(self):
        ...
        expect = pd.Series([log(i) for i in data], index=self.ticker).to_frame(name=self.index[-1]).T
        assert_frame_equal(self.market.daily_ret(), expect)
    def test_market_total_ret(self):
    ...
    def test_market_vol(self):
        def vol(data):
            ...
            return cal_std(ret)
        expect = pd.Series({k: vol(self.all_data[k]) for k in self.all_data}).to_frame(name=self.index[-1]).T
        print(expect)
        assert_frame_equal(self.market.volatility(), expect)
    def test_market_bollinger(self):
        def z_score(data):
            return (data[-1]-cal_mean(data))/cal_std(data)
        expect = pd.Series({k: z_score(self.all_data[k]) for k in self.all_data}).to_frame(name=self.index[-1]).T
        print(expect)
        assert_frame_equal(self.market.bollinger(), expect)
    def test_market_RSI(self):
        data = [1., 0., 0.5, 0.5, 0.5]
